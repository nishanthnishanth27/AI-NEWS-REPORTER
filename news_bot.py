...
def fetch_news():
    ...
    <head>
        ...
        <style>
            ...
            .news-box h3 {{ margin: 0 0 10px 0; font-size: 1.1em; color: #202124; }}
    ...
    try:
        ...
        res = requests.get("https://www.bbc.com/technology", headers=headers)
        soup = BeautifulSoup(res.content, 'html.parser')
        items = soup.find_all(['h2', 'h3'], limit=12)
        count = 0
        for item in items:
            title = item.get_text().strip()
            anchor = item.find_parent('a') or item.find('a')
            if anchor and anchor.has_attr('href') and len(title) > 20:
                link = anchor['href']
    ...
    try:
        ...
        res = requests.get("https://www.puthiyathalaimurai.com/tamilnadu", headers=headers)
        soup = BeautifulSoup(res.content, 'html.parser')
        # Finding all news cards
        cards = soup.find_all('div', class_='story-card-news-short-content-right-part', limit=10)
        if not cards: cards = soup.find_all('h2', limit=5)
        
        count = 0
        for card in cards:
            anchor = card.find('a') if card.name != 'a' else card
            title = card.get_text().strip()
            if anchor and anchor.has_attr('href') and len(title) > 15:
                link = anchor['href']
    ...
    try:
        ...
        res = requests.get("https://www.polimernews.com/category/tamilnadu", headers=headers)
        soup = BeautifulSoup(res.content, 'html.parser')
        items = soup.find_all('div', class_='category-news-content', limit=10)
        
        count = 0
        for item in items:
            anchor = item.find('a')
            title = item.find('h4').get_text().strip() if item.find('h4') else ""
            if anchor and anchor.has_attr('href') and title:
                link = anchor['href']
    ...
    print("✅ Full News with Links Updated!")
